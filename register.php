...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['fast_login'])) {
        ...
        if ($result['status'] == 'success') {
            ...
            $message = "Fast Login Failed. Try again.";
            $msg_type = "error";
        }
    } 
    // ======== NORMAL REGISTRATION LOGIC ========
    else if (isset($_POST['normal_register'])) {
        $name = $_POST['name'];
        $phone = $_POST['phone'];
        $pin = $_POST['pin'];
...
<head>
    ...
    <title>Register - Paikary Telecom</title>
...
<div class="max-w-[400px] w-full bg-white p-8 rounded-[40px] shadow-2xl relative overflow-hidden mx-4 my-8">
    <div class="absolute top-[-50px] right-[-50px] w-40 h-40 bg-purple-500 rounded-full mix-blend-multiply filter blur-2xl opacity-50"></div>
    <div class="absolute bottom-[-50px] left-[-50px] w-40 h-40 bg-pink-500 rounded-full mix-blend-multiply filter blur-2xl opacity-50"></div>
    ...
    <?php if($is_fast_login_success): ?>
        <div class="relative z-10 text-center py-4">
            ...
            <div class="bg-blue-50/50 p-5 rounded-3xl border border-blue-100 text-left mb-8 relative">
            ...
            <a href="index.php" class="block w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-green-500/40 transition-transform active:scale-95">
    ...
    <?php else: ?>
        <div class="relative z-10 text-center mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">Join Paikary Telecom today</p>
        </div>

        <?php if($message): ?>
            <div class="p-3 rounded-xl text-center mb-4 text-sm font-semibold border <?= $msg_type == 'success' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100' ?>">
                <?= $message ?>
            </div>
        <?php endif; ?>
        ...
        <div class="flex items-center my-4 relative z-10">
            ...
            <span class="px-4 text-xs text-gray-400 font-semibold uppercase">Or register manually</span>
            <div class="flex-grow h-px bg-gray-200"></div>
        </div>

        <form method="POST" class="space-y-4 relative z-10">
            <input type="hidden" name="normal_register" value="1">
            <div>
                ...
                <div class="relative mt-1">
                    ...
                    <input type="text" name="name" placeholder="Raj Ahmad" required class="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-purple-500 focus:outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative mt-1">
                    ...
                    <input type="tel" name="phone" placeholder="01XXXXXXXXX" required class="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-purple-500 focus:outline-none transition">
                </div>
            </div>

            <div>
                <label class="text-xs font-bold text-gray-500 uppercase ml-1">Set 4-Digit PIN</label>
                <div class="relative mt-1">
                    <i class="fa-solid fa-lock absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="password" name="pin" placeholder="••••" required maxlength="4" class="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-purple-500 focus:outline-none transition">
                </div>
            </div>

            <button type="submit" class="w-full mt-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-purple-500/40 transition-transform active:scale-95">
                Register Account
            </button>
        </form>

        <div class="mt-6 text-center relative z-10">
            <p class="text-sm text-gray-600">Already have an account? <a href="login.php" class="text-purple-600 font-bold hover:underline">Login</a></p>
        </div>
    <?php endif; ?>
</div>
</body>
